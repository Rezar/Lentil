...
typedef int32_t llama3_pos;
typedef int32_t llama3_seq_id;

// default hparams (LlaMa3)
struct llama3_hparams {
...
struct llama3_kv_cell {
    llama3_pos pos   = -1;
    llama3_pos delta = 0;

    std::set<llama3_seq_id> seq_id;

    bool has_seq_id(const llama3_seq_id & id) const {
        return seq_id.find(id) != seq_id.end();
    }
};

struct llama3_kv_cache {
    // key + value memory
    ...
    std::vector<llama3_kv_cell> cells;

    ggml_backend_buffer_t buffer;
};

struct llama3_model {
    llama3_hparams hparams;
    ...
    std::vector<llama3_layer> layers;

    llama3_kv_cache kv_cache;
...
// Input data for llama3_decode
// A llama3_batch object can contain input about one or many sequences
...
struct llama3_batch {
    ...
    float          * embd   = {};
    llama3_pos       * pos    = {};
    llama3_seq_id    * seq_id = {};
    int8_t         * logits = {};
};

// load the model's weights from a file
bool llama3_model_load(const std::string & fname, llama3_model & model, gpt_vocab & vocab, int n_ctx, int n_gpu_layers) {
...
struct ggml_cgraph * llama3_graph(
        const  llama3_model  & model,
        const  llama3_batch  & batch,
                     bool    measure) {
    const auto & hparams = model.hparams;
    ...
    static size_t buf_size = ggml_tensor_overhead()*LLAMA3_MAX_NODES + ggml_graph_overhead_custom(LLAMA3_MAX_NODES, false);
    ...
    struct ggml_cgraph  * gf = ggml_new_graph_custom(ctx, LLAMA3_MAX_NODES, false);
...
static void llama3_kv_cache_seq_cp(
        struct llama3_kv_cache & cache,
                 llama3_seq_id   seq_id_src,
                 llama3_seq_id   seq_id_dst,
                    llama3_pos   p0,
                    llama3_pos   p1) {
    if (p0 < 0) p0 = 0;
    if (p1 < 0) p1 = std::numeric_limits<llama3_pos>::max();
...
struct llama3_batch llama3_batch_init(int32_t n_tokens, int32_t embd) {
    llama3_batch batch;
    ...
    batch.pos    = (llama3_pos *)    malloc(sizeof(llama3_pos)    * n_tokens);
    batch.seq_id = (llama3_seq_id *) malloc(sizeof(llama3_seq_id) * n_tokens);
    batch.logits = (int8_t *)      malloc(sizeof(int8_t)      * n_tokens);

    return batch;
}

void llama3_batch_free(struct llama3_batch batch) {
...
// < 0 - error
int llama3_decode(
        struct llama3_model &  model,
        ggml_gallocr_t       allocr,
        struct llama3_batch    batch,
        int                  n_threads,
    ...
    struct ggml_cgraph * gf = llama3_graph(model, batch, false);
...

    {
        ...
        for (int h = 0; h < 1; ++h) {
            ...
            for (int j = 0; j < n_tokens; ++j) {
                const llama3_pos    pos    = batch.pos[j];
                const llama3_seq_id seq_id = batch.seq_id[j];
...
int main(int argc, char ** argv) {
    ...
    // load the model
    {
        ...
        if (!llama3_model_load(params.model, model, vocab, params.n_ctx, params.n_gpu_layers)) {
    ...
    llama3_batch batch = llama3_batch_init(n_batch_max, 0);
    ...
    ggml_gallocr_t allocr = NULL;
    {
        ...
        struct ggml_cgraph * gf = llama3_graph(model, batch, true);
    ...
    if (llama3_decode(model, allocr, batch, params.n_threads, logits) != 0) {
        printf("%s: llama3_decode() failed\n", __func__);
        return 1;
    }
    ...
    for (int32_t i = 1; i < n_parallel; ++i) {
        llama3_kv_cache_seq_cp(model.kv_cache, 0, i, 0, batch.n_tokens);
    }
    ...
    while (n_cur < n_len) {
...

        {
            ...
            int ret_code = llama3_decode(model, allocr, batch, params.n_threads, logits);
            if (ret_code != 0) {
